...
generalName = "result_trad_"+str(NN_type)+"_"+outputFileSufixTrain+"_batch_size_"+str(BATCH_SIZE)+"_window_"+str(TIME_SERIES_SIZE)+"-"+str(TIME_STEP_SHIFT)+"_max_ep_"+str(MAX_EPOCH)
...
# fit network
history = model.fit(train_dataset_series, epochs=epochs, verbose=verbose, batch_size=batch_size, callbacks=[checkpoint],validation_data=validation_dataset_series) #, batch_size=batch_size, validation_split=0.1
