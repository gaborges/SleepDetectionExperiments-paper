...
outputFileSufixTrain = "smote" # unbalanced file sufix is umb
...
X_test_data_df = X_test[inputFeatures]
y_test_label_df = X_test[outputClasses]

print("Test dataset")
print("Features: ",X_test_data_df.shape)
print("Output: ",y_test_label_df.shape)


# In[27]:


print("Transform the test data to series")

X_test_data, y_test_label = create_dataset_time_series_with_one_output_foward(    #timestamp
    X_test_data_df, 
    y_test_label_df, 
    TIME_SERIES_SIZE, 
    TIME_STEP_SHIFT
)
print("Size: ",X_test_data.shape,y_test_label.shape)     
...
for i in range(0,len(clientList)):
    ...
    client_train_dataset = tf.data.Dataset.from_tensor_slices((X_train_data, y_train_data))
    print("client "+str(i)+" shape: ",client_train_dataset.element_spec)    
    # apply the configs
...
def create_keras_model():
    return tf.keras.models.Sequential([
      tf.keras.layers.InputLayer(input_shape=(4,9,)),
