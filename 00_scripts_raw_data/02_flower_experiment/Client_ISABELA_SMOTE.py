...
X_train  = pd.read_csv(inputFolders+"student_"+train_folder+"_transformed_smote.csv")
...
def loadTestData(dataSet):
    ...
    for i in range(0,len(dataSet)):
        print(i , "-", dataSet[i],inputFolders+"student_"+dataSet[i]+"_transformed_smote.csv")
        #print(trainingDataSet[i])
        if(i == 0):
            X_test = pd.read_csv(inputFolders+"student_"+dataSet[i]+"_transformed_smote.csv")
        else:
            dataset = pd.read_csv(inputFolders+"student_"+dataSet[i]+"_transformed_smote.csv")
...
def evaluate_and_save_results(keras_model,X_test_data, y_test_label, current_round_index, clientId, prefix_string = "Results"):
    ...
    # write file
    outputMetricFile = "results-balanced/"+prefix_string+"_MLP_unbalanced_client_" + str(clientId) + "_round_" + str(current_round_index) + ".csv"
...
class CifarClient(fl.client.NumPyClient):
    ...
    def evaluate(self, parameters, config):
        ...
        file_global_model = "results-balanced/global_model_MLP_unbalanced_client_0_round_"+str(round_index)
